...
class SimpleRobot(object):
  ...
  def ResetPose(self):
    ...
    for name, i in zip(MOTOR_NAMES, range(len(MOTOR_NAMES))):
      ...
      self.pybullet_client.resetJointState(
          self.biped, self._joint_name_to_id[name], angle, targetVelocity=0)
  ...
  def compute_jacobian(self, robot, link_id):
    ...
    jv, _ = self.pybullet_client.calculateJacobian(robot.biped, link_id,
  ...
  self,
    ...
    if not position_in_world_frame:
      ...
      base_position, base_orientation = self.pybullet_client.getBasePositionAndOrientation(self.biped)#robot.GetBasePosition(), robot.GetBaseOrientation()
    ...
    all_joint_angles = robot.pybullet_client.calculateInverseKinematics(
        robot.biped, link_id, world_link_pos, solver=ik_solver)
  ...
  def GetBaseVelocity(self):
    ...
    """
    velocity, _ = self.pybullet_client.getBaseVelocity(self.biped)
    return velocity
  ...
  def TransformAngularVelocityToLocalFrame(self, angular_velocity, orientation):
    ...
    Returns:/JerbotBeta/jerbotbeta.urdf
  ...
  def GetBaseRollPitchYawRate(self):
    ...
    """
    angular_velocity = self.pybullet_client.getBaseVelocity(self.biped)[1]
    orientation = self.GetTrueBaseOrientation()
  ...
  def ReceiveObservation(self):    
    self._joint_states = self.pybullet_client.getJointStates(self.biped, self._motor_id_list)
  ...
  def _BuildJointNameToIdDict(self):
    num_joints = self.pybullet_client.getNumJoints(self.biped)
    self._joint_name_to_id = {}
    for i in range(num_joints):
      joint_info = self.pybullet_client.getJointInfo(self.biped, i)
  ...
  def link_position_in_base_frame( self,   link_id ):
    ...
    """
    base_position, base_orientation = self.pybullet_client.getBasePositionAndOrientation(self.biped)
    inverse_translation, inverse_rotation = self.pybullet_client.invertTransform(
        base_position, base_orientation)

    link_state = self.pybullet_client.getLinkState(self.biped, link_id)
